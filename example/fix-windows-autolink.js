...
// Ensure correct path to ReactNativeDeviceAi project (relative from example/windows/ReactNativeDeviceAiExample/)
...
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <ItemGroup>
    <ReactNativeModule Include="${correctProjectPath}">
...
console.log('\n4. Fixing duplicate MSBuild import warnings...');

// Read the current project file to check for duplicate imports
try {
  ...
  // Check if the AutolinkedNativeModules imports are in the correct place
  const hasAutolinkGroup = projectContent.includes('<ImportGroup Label="AutolinkedNativeModules">');
  
  if (hasAutolinkGroup) {
    console.log('✅ AutolinkedNativeModules ImportGroup already exists');
  } else {
    console.log('⚠️ AutolinkedNativeModules ImportGroup not found - project structure may be incorrect');
  }
  
  // Ensure the autolink imports are only included once and in the right place
  ...
  let hasCorrectImports = true;
  expectedImports.forEach(imp => {
    if (!projectContent.includes(imp)) {
      hasCorrectImports = false;
    }
  });
  
  if (hasCorrectImports) {
    console.log('✅ AutolinkedNativeModules imports are correctly configured');
  } else {
    console.log('⚠️ AutolinkedNativeModules imports may need adjustment');
  }
  
} catch (error) {
  console.log('❌ Error checking project file:', error.message);
}

console.log('\n5. Testing autolink functionality...');
try {
...
console.log('\n6. Creating build validation script...');
